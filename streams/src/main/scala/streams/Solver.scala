...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = 
    if initial.isEmpty then Nil.to(LazyList)
    else
      val frontier = 
        for 
          (block, moveList) <- initial
          newNeighbors <- newNeighborsOnly(neighborsWithHistory(block, moveList), explored)
        yield
          newNeighbors
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = 
    pathsFromStart.filter((x, _) => done(x))
  ...
  lazy val solution: List[Move] = 
    if pathsToGoal.isEmpty then Nil
