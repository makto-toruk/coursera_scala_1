...
trait DecoderInstances:
  ...
  given Decoder[String] =
    Decoder.fromPartialFunction { case Json.Str(s) => s.toString}
  
  /** A decoder for `Boolean` values */
  given Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(b) => if b then true else false}
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(x) => {
        val decoded = x.map(decoder.decode)
        if (decoded.forall(_.isDefined)) then Some(decoded.map(_.get)) else None
      }
      case _ => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(x) => {
        if x.contains(name) then decoder.decode(x(name)) else None
      }
      case _ => None
    }
